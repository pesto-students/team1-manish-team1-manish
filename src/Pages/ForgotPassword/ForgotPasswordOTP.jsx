...
const ForgotPasswordOTP = ({ email }) => {
  ...
  const handleOtp6Change = (value) => {
    ...
    setOtp(`${num1.current.value}${num2.current.value}${num3.current.value}${num4.current.value}${num5.current.value}${num6.current.value}`);
    num6.current.blur();
  }
  ...
  const validateOtp = async () => {
    ...
    await axios({
      method: 'post',
      url: process.env.REACT_APP_VALIDATE_OTP_URL,
      withCredentials: true,
      headers: {
        "Access-Control-Allow-Origin": process.env.REACT_APP_CORS_URL
      },
      ...
      <div className="forgot__password_container">
        <div className="forgot__password">
          ...
          <ThemeProvider theme={DarkTheme}>
            <Button onClick={validateOtp} className="mui-dark-btn" variant="contained">
              Submit
            </Button>
          </ThemeProvider>
